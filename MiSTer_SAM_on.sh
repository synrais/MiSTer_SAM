...
register_core "colecovision" "ColecoVision" "col,bin,rom" "_Console" "ColecoVision" "ColecoVision" "ColecoVision" "" 1 1 "f"
...
register_core "vectrex" "GCE Vectrex" "bin,vec" "_Console" "Vectrex" "Vectrex" "Vectrex" "" 1 1 "f"
...
function read_samini() {
	if [ ! -f "${samini_file}" ]; then
       ...
       # Remove any unregistered cores from the list to avoid failures
       local -a tmp_corelist=()
       for core in "${corelist[@]}"; do
               if [[ -v CORE_PRETTY["$core"] ]]; then
                       tmp_corelist+=("$core")
               else
                       samdebug "Unknown core '$core' in corelist - skipping"
               fi
       done
       corelist=("${tmp_corelist[@]}")
...
function filter_list() { # args: core
    ...
    if [ -s "${gamelistpath}/${core}_gamelist_exclude.txt" ]; then
        ...
        echo "No category excludelist found for '${core}'" >&2
    fi
    ...
    if [ -s "${gamelistpath}/${core}_excludelist.txt" ]; then
        ...
        echo "No excludelist file found for '${core}'" >&2
    fi
