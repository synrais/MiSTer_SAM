...
# Format: name|pretty|ext|pathfilter_var|pathrbf_var|launch|tty_pic|mgl_core
CORE_DATA=(
    "amiga|Commodore Amiga||amigapathfilter|amigapathrbf|Minimig|Minimig|Minimig"
    "amigacd32|Commodore Amiga CD32|chd,cue|amigacd32pathfilter|amigacd32pathrbf|Minimig|Minimig|Minimig"
    "ao486|PC 486 DX-100|mgl|ao486pathfilter|ao486pathrbf|ao486|ao486|ao486"
    "arcade|MiSTer Arcade|mra|arcadepathfilter|arcadepathrbf|Arcade|Arcade|Arcade"
    "atari2600|Atari 2600|a26|atari2600pathfilter|atari2600pathrbf|ATARI7800|ATARI2600|ATARI7800"
    "atari5200|Atari 5200|a52,car|atari5200pathfilter|atari5200pathrbf|ATARI5200|ATARI5200|ATARI5200"
    "atari7800|Atari 7800|a78|atari7800pathfilter|atari7800pathrbf|ATARI7800|ATARI7800|ATARI7800"
    "atarilynx|Atari Lynx|lnx|atarilynxpathfilter|atarilynxpathrbf|AtariLynx|AtariLynx|AtariLynx"
    "c64|Commodore 64|crt,prg|c64pathfilter|c64pathrbf|C64|C64|C64"
    "cdi|Philips CD-i|chd,cue|cdipathfilter|cdipathrbf|CDi|CD-i|CDi"
    "coco2|TRS-80 Color Computer 2|ccc|coco2pathfilter|coco2pathrbf|CoCo2|CoCo2|CoCo2"
    "colecovision|ColecoVision|col|colecovisionpathfilter|colecovisionpathrbf|ColecoVision|ColecoVision|ColecoVision"
    "fds|Nintendo Disk System|fds|fdspathfilter|fdspathrbf|NES|fds|NES"
    "gb|Nintendo Game Boy|gb|gbpathfilter|gbpathrbf|GAMEBOY|GAMEBOY|GAMEBOY"
    "gba|Nintendo Game Boy Advance|gba|gbapathfilter|gbapathrbf|GBA|GBA|GBA"
    "gbc|Nintendo Game Boy Color|gbc|gbcpathfilter|gbcpathrbf|GAMEBOY|GAMEBOY|GAMEBOY"
    "genesis|Sega Genesis / Megadrive|md,gen|genesispathfilter|genesispathrbf|MEGADRIVE|MegaDrive|MegaDrive"
    "gg|Sega Game Gear|gg|ggpathfilter|ggpathrbf|SMS|gamegear|SMS"
    "intellivision|Mattel Intellivision|int,bin,rom|intellivisionpathfilter|intellivisionpathrbf|Intellivision|Intellivision|Intellivision"
    "jaguar|Atari Jaguar|j64,rom,bin,jag|jaguarpathfilter|jaguarpathrbf|Jaguar|Jaguar|Jaguar"
    "megacd|Sega CD / Mega CD|chd,cue|megacdpathfilter|megacdpathrbf|MegaCD|MegaCD|MegaCD"
    "mgls|Custom MGL|mgl|mglspathfilter||MGL|MGL|"
    "n64|Nintendo N64|n64,z64|n64pathfilter|n64pathrbf|N64|N64|N64"
    "neogeo|SNK NeoGeo|neo|neogeopathfilter|neogeopathrbf|NEOGEO|NEOGEO|NEOGEO"
    "neogeocd|SNK NeoGeo CD|cue,chd|neogeocdpathfilter|neogeocdpathrbf|NEOGEO|NEOGEO|NEOGEO"
    "nes|Nintendo Entertainment System|nes|nespathfilter|nespathrbf|NES|NES|NES"
    "psx|Sony Playstation|chd,cue,exe|psxpathfilter|psxpathrbf|PSX|PSX|PSX"
    "s32x|Sega 32x|32x|s32xpathfilter|s32xpathrbf|S32X|S32X|S32X"
    "saturn|Sega Saturn|cue,chd|saturnpathfilter|saturnpathrbf|SATURN|SATURN|SATURN"
    "sgb|Super Gameboy|gb,gbc|sgbpathfilter|sgbpathrbf|SGB|SGB|SGB"
    "sms|Sega Master System|sms,sg|smspathfilter|smspathrbf|SMS|SMS|SMS"
    "snes|Super Nintendo|sfc,smc|snespathfilter|snespathrbf|SNES|SNES|SNES"
    "stv|Sega Titan Video||stvpathfilter|stvpathrbf|S-TV|S-TV|S-TV"
    "tgfx16|NEC TurboGrafx-16 |pce,sgx|tgfx16pathfilter|tgfx16pathrbf|TGFX16|TGFX16|TurboGrafx16"
    "tgfx16cd|NEC TurboGrafx-16 CD|chd,cue|tgfx16cdpathfilter|tgfx16cdpathrbf|TGFX16|TGFX16|TurboGrafx16"
    "vectrex|GCE Vectrex|bin|vectrexpathfilter|vectrexpathrbf|Vectrex|Vectrex|Vectrex"
    "wonderswan|Bandai WonderSwan|ws|wonderswanpathfilter|wonderswanpathrbf|WonderSwan|WonderSwan|WonderSwan"
    "wonderswancolor|Bandai WonderSwan Color|wsc|wonderswancolorpathfilter|wonderswancolorpathrbf|WonderSwan|WonderSwan|WonderSwan"
    "x68k|Sharp X68000|mgl|x68kpathfilter|x68kpathrbf|X68000|X68000|X68000"
)
...
function init_vars() {
	...
	if [[ -n "$cfgcore_configpath" ]]; then
        ...
        # Populate core configuration arrays from CORE_DATA
        declare -gA CORE_PRETTY CORE_EXT PATHFILTER CORE_PATH_RBF CORE_LAUNCH TTY2OLED_PIC_NAME MGL_CORE
        local entry core pretty ext pf_var rb_var launch tty mgl pf_val rb_val
        for entry in "${CORE_DATA[@]}"; do
                IFS='|' read -r core pretty ext pf_var rb_var launch tty mgl <<< "${entry}"
                CORE_PRETTY["$core"]="$pretty"
                CORE_EXT["$core"]="$ext"
                [[ -n $pf_var ]] && pf_val="${!pf_var}" || pf_val=""
                [[ -n $rb_var ]] && rb_val="${!rb_var}" || rb_val=""
                PATHFILTER["$core"]="$pf_val"
                CORE_PATH_RBF["$core"]="$rb_val"
                CORE_LAUNCH["$core"]="$launch"
                TTY2OLED_PIC_NAME["$core"]="$tty"
                MGL_CORE["$core"]="$mgl"
        done
